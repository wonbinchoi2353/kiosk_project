...
- 8/3 옵션 테이블 모든 정보 메모리에 캐싱 node-cache
- 8/4 상품 조회 api 수정, 상품 주문 api 수정
...
| 고객 상품 결제 | `put`    | api/orders/:order_customer_id |
...
- item_id 지우고 시도중
- order_item_id 사용해서 item 있는지 확인
...
### (고객 상품 주문) order_customer_id를 params로 받지 않고 주문 완료 됐을 때 반환만 해야할지
...
### (고객 상품 주문) repository, service 어느쪽에 비즈니스 로직 만들지
...
### (고객 상품 주문) 한 명이 주문 하면 order_customer를 하나만 만드는 방법
...
- 1:1예시 (아메리카노: 엑스트라 500, 샷 추가 300, 핫/아이스), (수박주스: 엑스트라 500, 샷 추가 없음)

### findByPk(매개변수) 에 대한 오해

- 매개변수가 키와 이름이 같아야 하는 줄 알았는데 매개변수 이름은 아무런 상관이 없는 것 같다. (pk 이름이 id라면 매개변수도 id라고 해야만 하는줄 알았다)

### (고객 상품 주문) 주문서(item_order_customer)에 어떤 옵션을 저장할지

- 옵션 테이블과 아이템 테이블이 1:n 또는 n:1 관계라면 고객은 옵션 번호만 선택하기 때문에 옵션 테이블에 있는 옵션 로우를 주문서의 옵션 (json) 컬럼에 그대로 저장하면 된다
- 옵션 테이블과 아이템 테이블이 1:1 관계라면 고객이 옵션을 개별로 선택하기 때문에 옵션 테이블을 가져와서 고객이 선택한 옵션을 계산 후 계산한 값을 주문서의 옵션 (json) 컬럼에 저장한다.

### (고객 상품 주문) 같은 api에서 주문서 만든 뒤 바로 조회하면 아직 없어서 조회가 안 된다?

- order_customer create: -> 결과값 변수로 받으면 값이 존재함
- item_order_customer: create -> findAll 없음
- body에서 받은 items 배열을 forEach로 순회하면서 forEach 안에 있는 db를 사용하기 위해 새로운 async를 썼는데 결국 forEach 문의 모든 코드가 가장 나중에 실행되면서 밑에 있는 코드들이 데이터가 아직 만들어지지 않은 상태에서 실행되었다.
- forEach 같은 함수 대신 for of 문을 사용해 async를 만들지 않아도 되게끔 했다.

### (고객 상품 주문) 각각의 주문서를 어떻게 응답할지

- 주문서1(+아이템 이름), 주문서2(+아이템 이름), 총액
- 응답할 주문서를 하나씩 가공해보려고 했는데 어려워서 일단 주문 번호와 총액만 전달해야 할 것같다

### X (고객 주문 수정) 옵션을 입력할 수 있게 만들어서 수정하기 까다로워진 것 같다

- 먼저 주문했던 데이터 보여주기 (item_order_customer 사용)
- 해당 데이터에서 상태를 수정할 주문서 번호와 수정할 데이터만 입력 (1번 주문의 2번 주문서 수정)
- 입력된 데이터만 수정
- 주문 수정이 주문의 상태만 수정하는 겉 같다.

### 고객 결제에서 트랜잭션이 필요한 부분

- 결제 했을 때 아이템이 부족하면 결제 취소
